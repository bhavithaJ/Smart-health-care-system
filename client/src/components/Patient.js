...
const Patient = ({SmartH, account, ethValue}) => {
  ...
  const getPatientData = async () => {
      var patient = await SmartH.methods.patientInfo(account).call();
      setPatient(patient);
  }
  const giveAccess = (e) => {
    e.preventDefault();
    SmartH.methods.permitAccess(docEmail).send({from: account}).on('transactionHash', (hash) => {
      return window.location.href = '/login'
    })
  }
  const revokeAccess = async (email) => {
    var addr = await SmartH.methods.emailToAddress(email).call();
    SmartH.methods.revokeAccess(addr).send({from: account}).on('transactionHash', (hash) => {
      return window.location.href = '/login';
    });
  }
  const getDoctorAccessList = async () => {
    var doc = await SmartH.methods.getPatientDoctorList(account).call();
    let dt = [];
    for(let i=0; i<doc.length; i++){
      let doctor = await SmartH.methods.doctorInfo(doc[i]).call();
      dt = [...dt, doctor]
    }
    setDocList(dt)
  }
  const getInsurer = async () => {
    var insurer = await SmartH.methods.insurerInfo(patient.policy.insurer).call();
    setInsurer(insurer)
  }
  const getInsurerList = async () => {
    var ins = await SmartH.methods.getAllInsurersAddress().call();
    let it = [];
    for(let i=0; i<ins.length; i++){
      let insurer = await SmartH.methods.insurerInfo(ins[i]).call();
  ...
  const getPolicyList = async () => {
    var policyList = await SmartH.methods.getInsurerPolicyList(buyFromInsurer).call()
    setPolicyList(policyList);
  }
  const purchasePolicy = async (e) => {
    e.preventDefault();
    var value = policyList[buyPolicyIndex].premium/ethValue;
    SmartH.methods.buyPolicy(parseInt(policyList[buyPolicyIndex].id)).send({from: account, value: Web3.utils.toWei(value.toString(), 'Ether')}).on('transactionHash', (hash) => {
      return window.location.href = '/login'
    })
  }
  const getTransactionsList = async () => {
    var transactionsIdList = await SmartH.methods.getPatientTransactions(account).call();
    let tr = [];
    for(let i=transactionsIdList.length-1; i>=0; i--){
        let transaction = await SmartH.methods.transactions(transactionsIdList[i]).call();
        let doctor = await SmartH.methods.doctorInfo(transaction.receiver).call();
  ...
  const settlePayment = async (e, transaction) => {
    let value = transaction.value/ethValue;
      SmartH.methods.settleTransactionsByPatient(transaction.id).send({from: account, value: Web3.utils.toWei(value.toString(), 'Ether')}).on('transactionHash', (hash) => {
        return window.location.href = '/login'
    })
  }
