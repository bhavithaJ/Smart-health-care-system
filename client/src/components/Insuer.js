...
const Insurer = ({SmartH, account, ethValue}) => {
    ...
    const getInsurerData = async () => {
        var insurer = await SmartH.methods.insurerInfo(account).call();
        setInsurer(insurer);
    }
    const getPolicyList = async () => {
        var pol = await SmartH.methods.getInsurerPolicyList(account).call();
        setPolicyList(pol)
    }
    const createPolicy = (e) => {
        e.preventDefault()
        SmartH.methods.createPolicy(polName, polCoverValue, polDuration, polPremium).send({from: account}).on('transactionHash', (hash) => {
    ...
    const getPatientList = async () => {
        var pat = await SmartH.methods.getInsurerPatientList(account).call();
        let pt = [];
    ...
    const getClaimsData = async () => {
        var claimsIdList = await SmartH.methods.getInsurerClaims(account).call();
        let cl = [];
        for(let i=claimsIdList.length-1; i>=0; i--){
            let claim = await SmartH.methods.claims(claimsIdList[i]).call();
            let patient = await SmartH.methods.patientInfo(claim.patient).call();
            let doctor = await SmartH.methods.doctorInfo(claim.doctor).call();
    ...
    const approveClaim = async (e, claim) => {
        ...
        SmartH.methods.approveClaimsByInsurer(claim.id).send({from: account, value: Web3.utils.toWei(value.toString(), 'Ether')}).on('transactionHash', (hash) => {
            return window.location.href = '/login'
        })
    }
    const rejectClaim = async (e, claim) => {
        SmartH.methods.rejectClaimsByInsurer(claim.id).send({from: account}).on('transactionHash', (hash) => {
            return window.location.href = '/login'
        })
    }
                ...
                <div className='box'>
                    ...
                    <Form>
                        ...
                        <Form.Group className='mb-3'>
                            ...
                            <Form.Control required type="number" value={polCoverValue} onChange={(e) => setPolCoverValue(e.target.value)} placeholder='Enter the cover value in USD'></Form.Control>
                        </Form.Group>
                        <Form.Group className='mb-3'>
                            <Form.Label>Yearly Premium: </Form.Label>
                            <Form.Control required type="number" value={polPremium} onChange={(e) => setPolPremium(e.target.value)} placeholder='Enter the annual premium in USD'></Form.Control>
                        </Form.Group>
                        <Form.Group className='mb-3'>
                            ...
                            { policyList.length > 0 ?
                                policyList.map((pol, idx) => {
                                    ...
                                    <tr key={idx+1}>
                                        <td>{idx+1}</td>
                                        <td>{pol.name}</td>
                                        <td>USD {pol.coverValue}</td>
                                        <td>USD {pol.premium}/year</td>
