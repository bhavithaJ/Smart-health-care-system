...
const Login = ({SmartH, connectWallet, token, account, setToken, setAccount}) => {
    const navigate = useNavigate();

    const handleSubmit = async (e) => {
        e.preventDefault();
        if(account==="") return;
        var res = await SmartH.methods.patientInfo(account).call()
        if(res.exists){
            setToken('1');
        ...
        res = await SmartH.methods.doctorInfo(account).call()
        if(res.exists){
            setToken('2');
        ...
        res = await SmartH.methods.insurerInfo(account).call()
        if(res.exists){
            setToken('3');
    ...
    useEffect(() => {
        if(SmartH){   
            ...
            if((t!=="" || a!=="") && (a===account || account==='')){
                if(t==="1"){
                    SmartH.methods.patientInfo(a).call().then((res) => {
                        if(res.exists){
                            setToken(t);
                            setAccount(a);            
                            return navigate('/dashboard')
                    ...
                    SmartH.methods.doctorInfo(a).call().then((res) => {
                        if(res.exists){
                            setToken(t);
                            setAccount(a);
                            return navigate('/dashboard')
                    ...
                    SmartH.methods.insurerInfo(a).call().then((res) => {
                        if(res.exists){
                            setToken(t);
                            setAccount(a);
                            return navigate('/dashboard')
